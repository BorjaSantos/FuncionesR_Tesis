...
# m6.trat / m6.trat: media grupo tratamiento / media grupo control para la variable 6.
# sd6.trat / sd6.trat: desviacion estandar grupo tratamiento / desviacion estandar grupo control para la variable 6.
# a6 / b6: limites superior e inferior del rango de valores de la variable 6. 
...
simulacion_datos <- function(n.estudios,m1.trat,sd1.trat,m1.ctrl,sd1.ctrl,a1,b1,m2.trat,sd2.trat,m2.ctrl,sd2.ctrl,a2,b2,
                             ...
                             m6.trat,sd6,trat,m6.trat,m6.ctrl,tamano.muestra,semilla,
                             replicaciones){
  ...
  for(i in 1:replicaciones){
    ...
    for(j in 1:n.estudios){
      Var1.Trat <- round(rtrunc(tamano.muestra,spec="norm",a=a1,b=b1,mean=m1.trat,sd=sd1.trat),0) # Puntuaciones variable 1 tratamiento
      Var1.Ctrl <- round(rtrunc(tamano.muestra,spec="norm",a=a1,b=b1,mean=m1.ctrl,sd=sd1.ctrl),0) # Puntuaciones variable 1 control
      ...
      Var5.Trat <- round(rtrunc(tamano.muestra,spec="norm",a=a5,b=b5,mean=m5.trat,sd=sd5.trat),0) # Puntuaciones variable 5 tratamiento
      Var5.Ctrl <- round(rtrunc(tamano.muestra,spec="norm",a=a5,b=b5,mean=m5.ctrl,sd=sd5.ctrl),0) # Puntuaciones variable 5 control
      Var6.Trat <- round(rtrunc(tamano.muestra,spec="norm",a=a6,b=b6,mean=m6.trat,sd=sd6.trat),0) # Puntuaciones variable 6 tratamiento
      Var6.Ctrl <- round(rtrunc(tamano.muestra,spec="norm",a=a6,b=b6,mean=m6.ctrl,sd=sd6.ctrl),0) # Puntuaciones variable 6 control
...
g_hedges_simulacion2_6vars <- function(database,correlacion12=0,
  ...
  library(compute.es)
  datos_meta <- diag(0,replicaciones,27)
  for(i in 1:replicaciones){
    ...
    g6 <- mes(mean(database[[i]][,11]),sd(database[[i]][,11]),dim(database[[i]])[1],mean(database[[i]][,12]),sd(database[[i]][,12]),dim(database[[i]])[1])[12][,1]
    ...
    var.g6 <- mes(mean(database[[i]][,11]),sd(database[[i]][,11]),dim(database[[i]])[1],mean(database[[i]][,12]),sd(database[[i]][,12]),dim(database[[i]])[1])[13][,1]
...
g_hedges_simulacion_6vars <- function(database,correlacion12=0,
                                      correlacion13=0,correlacion14=0,correlacion15=0,correlacion16=0,correlacion23=0,
                                      correlacion24=0,correlacion25=0,correlacion26=0,correlacion34=0,correlacion35=0,
                                      correlacion36=0,correlacion45=0,correlacion46=0,correlacion56=0){
  library(compute.es)
  ...
  g6 <- vector(mode="numeric",length=length(database[[1]]))
  ...
  var.g6 <- vector(mode="numeric",length=length(database[[1]]))
  ...
  covar.g1g6 <- vector(mode="numeric",length=length(database[[1]])) 
  ...
  covar.g2g6 <- vector(mode="numeric",length=length(database[[1]])) 
  covar.g3g4 <- vector(mode="numeric",length=length(database[[1]])) 
  covar.g3g5 <- vector(mode="numeric",length=length(database[[1]])) 
  covar.g3g6 <- vector(mode="numeric",length=length(database[[1]])) 
  covar.g4g5 <- vector(mode="numeric",length=length(database[[1]])) 
  covar.g4g6 <- vector(mode="numeric",length=length(database[[1]])) 
  covar.g5g6 <- vector(mode="numeric",length=length(database[[1]])) 
  for(i in 1:length(database)){
    for(j in 1:length(database[[i]])){
      ...
      g6[j] <- mes(mean(database[[i]][[j]][,11]),sd(database[[i]][[j]][,11]),dim(database[[i]][[j]])[1],mean(database[[i]][[j]][,12]),sd(database[[i]][[j]][,12]),dim(database[[i]][[j]])[1])[12][,1]
      ...
      var.g6[j] <- mes(mean(database[[i]][[j]][,11]),sd(database[[i]][[j]][,11]),dim(database[[i]][[j]])[1],mean(database[[i]][[j]][,12]),sd(database[[i]][[j]][,12]),dim(database[[i]][[j]])[1])[13][,1]
...
# datos <- g_hedges_simulacion2_6vars(database,correlacion12=0,correlacion13=0,correlacion14=0,correlacion15=0,correlacion16=0,
#                                     correlacion23=0,correlacion24=0,correlacion25=0,correlacion26=0,correlacion34=0,
#                                     correlacion35=0,correlacion36=0,correlacion45=0,correlacion46=0,correlacion56=0)
...
# datos <- g_hedges_simulacion_6vars(database,correlacion12=0,correlacion13=0,correlacion14=0,correlacion15=0,correlacion16=0,
#                                   correlacion23=0,correlacion24=0,correlacion25=0,correlacion26=0,correlacion34=0,
#                                   correlacion35=0,correlacion36=0,correlacion45=0,correlacion46=0,correlacion56=0)
